...
from autotest.forms import AddCodeForm, EditCodeForm
from autotest.models.codes import Code
from autotest.models.admin import ModuleType
from autotest.extensions import db, up
...
def upload_file(AddCodeForm):
    ...
    os.chdir(sys_path)
    os.system("mv " + filename + " test." + filename.rsplit('.', 1)[1])

    if filename.rsplit('.', 1)[1] in ["zip"]:
        cmd1 = "unzip test.zip"
    elif filename.rsplit('.', 1)[1] in ["tar"]:
        cmd1 = "tar -xf test.tar"
    os.system(cmd1)
    os.system("rm test." + filename.rsplit('.', 1)[1])
    os.system("mv * test/")
    os.chdir(sys_path + '/test')
    os.system("hg init")
    os.system("hg add *")
        ...
        try:
            if form.path.has_file():
                try:
                    ...
                    flash(_(u"错误的压缩文件内容，请重新上传"), "error")
            ...
            db.session.add(code)
            db.session.commit()
            flash(_(u"Add {0} successfully!".format(code.name)), "success")
            return redirect(url_for("code.showcode"))
        except:
            flash(_(u"错误的文件类型，只允许ZIP或TAR"), "error")
            return redirect(url_for("code.addcode"))
    parents = ModuleType.query.get_parent()
    nodes = ModuleType.query.get_allsubmodule()
...
def delcode(code_id):
    ...
    flash(_(u"{0} has been deleted".format(code.name)), "success")
    return redirect(url_for("code.showcode"))
...
def editcode(code_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(code)
        db.session.commit()
        flash(_(u"Update {0} informatiion successfully".format(code.name)),
              "success")
        return redirect(url_for("code.showcode"))
    parents = ModuleType.query.get_parent()
    nodes = ModuleType.query.get_allsubmodule()
