...
from autotest.forms import LoginForm, ChangePasswordForm, SignupForm,\
    EditMyAccountForm
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and auth:
            ...
            #print dir(current_app._get_current_object())
            #need add next url
            flash(_(u"welcome {0}".format(user.username)), "success")
            return redirect(url_for('index.index'))
        else:
            flash(_(u"无效登陆，请检查用户名或者密码"), "error")
...
def logout():
    flash(_("You are logout"), "success")
    identity_changed.send(current_app._get_current_object(),
                          identity=AnonymousIdentity())
...
def edit():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(_("Update user information successfully"), "success")
        return redirect(url_for("account.login"))
