...
from flask import Module, flash, g, redirect, url_for, render_template
from autotest.forms import AddCaseForm, EditCaseForm
...
def addcase():
    ...
    if form.validate_on_submit():
        ...
        if case.module_type:
            ...
            db.session.add(case)
            db.session.commit()
            flash(_(u"Add {0} successfully!".format(case.title)), "success")
            return redirect(url_for("case.showcase"))
    parents = ModuleType.query.get_parent()
    nodes = ModuleType.query.get_allsubmodule()
...
def delcase(case_id):
    ...
    db.session.commit()
    flash(_(u"{0} has been deleted".format(case.title)), "success")
    return redirect(url_for("case.showcase"))
...
def editcase(case_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(case)
        db.session.commit()
        flash(_(u"Update {0} successfully!".format(case.title)), "success")
        return redirect(url_for("case.showcase"))
    parents = ModuleType.query.get_parent()
    nodes = ModuleType.query.get_allsubmodule()
...
def copycase(case_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(_(u"Copy {0} successfully!".format(new_case.title)), "success")
        return redirect(url_for("case.showcase"))
    parents = ModuleType.query.get_parent()
    nodes = ModuleType.query.get_allsubmodule()
    return render_template('case/editcase.html', form=form, parents=parents,
