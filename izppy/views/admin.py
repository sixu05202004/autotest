...
from izppy.forms import EditAccountForm, AddModuleForm, \
...
def deluser(user_id):
    ...
    if user_id == g.user.id:
        ...
        identity_changed.send(current_app._get_current_object(),
                            identity=AnonymousIdentity())
        """
        flash(u"{0} has been deleted".format(user.username), "success")
...
def edituser(user_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        if user_id == g.user.id:
            flash(u"Update your account successfully", "success")
            return redirect(url_for("account.login"))
        else:
            flash(u"Update {0} successfully".format(user.username),
                  "success")
    else:
        parents = ModuleType.query.get_parent()
        # return redirect(url_for("admin.showuser"))
